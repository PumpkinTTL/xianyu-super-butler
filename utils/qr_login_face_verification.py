#!/usr/bin/env python3
"""
闲鱼扫码登录 - 人脸验证处理（纯 API 链路）

参照 zhinianboke/xianyu-auto-reply 的抓包实现，纯 API 复现浏览器端人脸验证流程：
1. 跟随 iframe_url(302) 落到 normal_validate.htm，提取 htoken
2. 从 normal_validate.htm 提取下一跳 verify_modes.htm 链接(补 _umidfg=1)
...
不需要 Playwright，不需要打码平台，人脸由用户本人扫码完成。
"""
...
async def run_face_verification(manager, session_id: str, iframe_url: str) -> None:
    """
    自动处理人脸验证链路（后台任务）

    Args:
        manager: 二维码登录管理器实例(提供 headers/timeout/proxy)
        session_id: 二维码会话 ID
    ...
    # 人脸验证额外延长会话有效期（手机端操作需要时间）
    session.created_time = time.time()
    session.expire_time = 900  # 15分钟
    ...
    try:
        ...
        async with httpx.AsyncClient(
            ...
            if not face_qr_match:
                raise RuntimeError("人脸验证：未能提取人脸验证二维码 URL")
            face_qr_content = face_qr_match.group(1)
            face_qr_url = render_qr_base64(face_qr_content)
            session.face_qr_url = face_qr_url
            logger.info(f"人脸验证二维码已生成: {session_id}")
            ...
            api_face_check = f"{manager.host}/iv/photoVerify/check.do"
